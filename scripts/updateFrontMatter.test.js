...
Content`;
  ...
  it('updates front matter with modified date', async () => {
    ...
    expect(fs.writeFile).toHaveBeenCalledWith(filePath, expect.stringContaining('modified: 2022-01-01T00:00:00.000Z'));
  });
...
Content`;
  ...
  it('supports custom date format', async () => {
    ...
    expect(fs.writeFile).toHaveBeenCalledTimes(1);
    expect(fs.writeFile).toHaveBeenCalledWith(filePath, expect.stringContaining('modified: Sat, 01 Jan 2022 00:00:00 GMT'));
  });

  it('supports custom date property', async () => {
    await updateFrontMatter(filePath, { dateProperty: 'updated' });
    expect(fs.writeFile).toHaveBeenCalledTimes(1);
    expect(fs.writeFile).toHaveBeenCalledWith(filePath, expect.stringContaining('updated: 2022-01-01T00:00:00.000Z'));
  });
