...
export const generateDefinitions = ({rowsPerPage}) => {
	...
	definitions.views = definitions.views.map(
		view => {
...
export const generateDataRow = n => {
	return(
	{
		"name": {
			"value": "Dummy service " + n
		},
		"comment": {
			"value": lipsum(n)
		},
...
const lipsum = numSentences => LOREMIPSUM.split('.').slice(numSentences,numSentences*2).join('.') + '.'