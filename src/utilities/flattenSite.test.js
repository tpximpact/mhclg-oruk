...
describe('flattenSite', () => {
	...
	test('should handle multiple levels of nesting', () => {
		...
		expect(result.find(item => item.name === 'level2')).toEqual(
			expect.objectContaining({
				name: 'level2',
	...
	test('should handle offsite links correctly', () => {
		...
		expect(result.find(item => item.name === 'external')).toEqual(
			expect.objectContaining({
				name: 'external',
	...
	test('should preserve all properties of nodes', () => {
		...
		expect(result.find(item => item.name === 'page')).toEqual(
			expect.objectContaining({
				customProp: 'value'
			})
		)
		expect(result.find(item => item.name === 'subpage')).toEqual(
			expect.objectContaining({
				anotherProp: 123
			})
		)
