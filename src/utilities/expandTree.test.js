...
describe('expandTree', () => {
  ...
  test('should process nested structure with parent nodes', () => {
    ...
    const result = expandTree(structure)
    expect(result).toEqual([
      { name: 'child1', urlPath: 'parent-path/child1-path', parentNodeName: 'parent', childNodes: undefined },
      { name: 'child2', urlPath: 'parent-path/child2-path', parentNodeName: 'parent', childNodes: undefined },
  ...
  test('should handle offsite nodes correctly', () => {
    ...
    const result = expandTree(structure)
    expect(result).toEqual([
      { name: 'child1', urlPath: 'https://external.com', offsite: true, parentNodeName: 'parent', childNodes: undefined },
      { name: 'child2', urlPath: 'parent-path/child2-path', parentNodeName: 'parent', childNodes: undefined },
      { name: 'parent', urlPath: 'parent-path', childNodes: undefined }
    ])
  })
  ...
  test('should handle deeply nested structures', () => {
    ...
    const result = expandTree(structure)
    expect(result).toEqual([
      { name: 'level2', urlPath: 'root/level1/level2', parentNodeName: 'level1', childNodes: undefined },
