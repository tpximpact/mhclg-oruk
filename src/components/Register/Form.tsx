...
const TextField = ({ id, label, note, formState }: FieldProps) => {
  const v = (formState?.formData?.[id] as string) ?? ''
...
const TextArea = ({ id, label, note, formState }: FieldProps) => {
  const v = (formState?.formData?.[id] as string) ?? ''
  ...
  return (
    <div>
      ...
      <p style={{ margin: '2rem 0' }}>
        <strong>What happens next</strong>: Our team will review your feed. This is a human-moderated
        process and may take up to 28 days. When the review process is complete, we will send an email to
        the contact email address supplied.
      </p>
      <p>
...
export const Form = () => {
  const [formState, action] = useActionState<any, FormData>(createMessage as any, EMPTY_FORM_STATE as any)
