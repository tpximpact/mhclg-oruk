...
const Headline = ({ text }) => <h3><strong>{text}</strong></h3>

const Bodycopy = ({ md, pad }) => {
	const parsed = parseMarkdown(md)
	const html = parsed.content
	return <div style={{padding: pad ? "2rem" : 0, fontWeight:"700"}} className={styles.content} dangerouslySetInnerHTML={{ __html: html }} />
}
