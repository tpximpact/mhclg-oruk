// accesible pagination adapted from
// https://github.com/mdn/css-examples/blob/main/css-cookbook/pagination--download.html

import styles from './Pagination.module.css'
import { ReactNode } from 'react'

interface ItemProps {
  enabled?: boolean
  children: ReactNode
  currentPage: number
  pageNumber: number
  pageChangeFunction: (baseUrl: string, pageNumber: number) => void
  baseUrl: string
}

interface PaginationItemProps {
  currentPage: number
  numPages: number
  pageChangeFunction: (baseUrl: string, pageNumber: number) => void
  baseUrl: string
}

const PreviousItem = (props: PaginationItemProps) => (
  <Item {...props} pageNumber={props.currentPage - 1} enabled={props.currentPage !== 1}>
    <span aria-hidden='true'>« previous</span>
    <span className={styles.visuallyhidden}>previous set of pages</span>
  </Item>
)

const NextItem = (props: PaginationItemProps) => (
  <Item {...props} pageNumber={props.currentPage + 1} enabled={props.currentPage < props.numPages}>
    <span className={styles.visuallyhidden}>next set of pages</span>
    <span aria-hidden='true'>next »</span>
  </Item>
)

const NumberedItem = (props: PaginationItemProps & { pageNumber: number }) => (
  <Item {...props}>
    <span className={styles.visuallyhidden}>page </span>
    {props.pageNumber}
  </Item>
)

const Item = ({
  enabled = true,
  children,
  currentPage,
  pageNumber,
  pageChangeFunction,
  baseUrl
}: ItemProps) => {
  const ariaProps: { 'aria-current'?: 'page' } = {}
  if (pageNumber === currentPage) {
    ariaProps['aria-current'] = 'page'
  }

  return (
    <li {...ariaProps}>
      {' '}
      {enabled ? (
        <span>
          <a
            href={baseUrl + pageNumber}
            onClick={e => {
              pageChangeFunction(baseUrl, pageNumber)
              e.preventDefault()
            }}
          >
            {children}
          </a>
        </span>
      ) : (
        <span className={styles.hide}>{children}</span>
      )}
    </li>
  )
}

export const Pagination = (props: PaginationItemProps) => {
  return (
    <nav className={styles.pagination} aria-label='pagination'>
      <ol>
        <PreviousItem {...props} />

        {[...Array(props.numPages)].map((_, i) => (
          <NumberedItem key={i} pageNumber={i + 1} {...props} />
        ))}

        <NextItem {...props} />
      </ol>
    </nav>
  )
}
